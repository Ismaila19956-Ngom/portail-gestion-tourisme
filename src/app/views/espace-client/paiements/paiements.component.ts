...
export class PaiementsComponent implements OnInit {
    ...
    private _toRow(p: any): PaiementRow {
        ...
        return {
            ...
            policeNum:     p.numeroPolice || p.police?.numeroPolice || 'â€”',
            produitNom:    p.nomPolice || p.police?.produit?.nom || 'â€”',
            dateEmission:  p.dateEmission || '',
            montantPaye:   p.montantPaye  || 0,
            modeLabel:     m ? (m.description || m.libelle || m.name || String(m)) : 'â€”',
            encaisse:      !!p.encaisse,
            statutLabel:   p.statut?.description || p.statut?.name || String(p.statut || 'â€”'),
            statutDot,

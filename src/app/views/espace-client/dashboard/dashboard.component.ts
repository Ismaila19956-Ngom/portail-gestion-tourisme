    ...
    <div class="dash">
        ...
        <div *ngIf="alertsOpen && alerts.length > 0" class="alert-panel">
            <div class="alert-panel-header">
                ...
                <button (click)="alertsOpen=false" style="background:none;border:none;cursor:pointer;color:#aaa;font-size:1.1rem;">✕</button>
            </div>
        ...
        <div class="hero">
            ...
            <div class="hero-left">
                ...
                <p class="hero-sub">Votre espace Sénégal Excursions — {{ today }}</p>
                <div class="hero-stats">
                    <div class="hero-stat">
                        <span class="hero-stat-val">{{ featuredCard ? 1 : 0 }}</span>
                        <span class="hero-stat-lbl">Derni�re police</span>
                    </div>
        ...
        <ng-container *ngIf="!loading">
            ...
            <!-- ══ POLICE LA PLUS R�CENTE (vedette) ══ -->
                ...
                <div class="featured-card" [style.background]="'linear-gradient(135deg,' + featuredCard.gradientFrom + ' 0%,' + featuredCard.gradientTo + ' 100%)'">
                    ...
                    <div class="fc-body">
                        <div class="fc-info-grid">
                            ...
                            <div class="fc-info-block">
                                <div class="fc-info-lbl">Capital assur�</div>
                    ...
                    <div class="fc-footer">
                        <a [routerLink]="['/mon-espace/polices', featuredCard.id]" class="fc-btn">
                            <i class="fa-solid fa-eye"></i> Consulter le d�tail
                        </a>
...
export class DashboardComponent implements OnInit {
    ...
    private static readonly GRADIENTS: [string, string][] = [
        ...
        ['#1a6b3c','#0f4d26'],  // vert for�t
        ['#2e7d32','#1b5e20'],  // vert fonc�
        ['#006064','#00838f'],  // teal
        ['#1565c0','#0d47a1'],  // bleu
    ];
    ...
    ngOnInit() {
        ...
        this.portal.getPolices(clientId, 0, 8).subscribe({
            next: res => {
                ...
                this.featuredCard = cards.length > 0 ? cards[0] : null;  // la plus r�cente
    ...
    private toCard(p: any, gi: number): PoliceCard {
        ...
        // Couleur dot statut selon �tat
        const statutDot = active ? '#4ade80'
            : sN === 'RESILIE' || sN === 'EXPIRE' ? '#f87171'
            : sN === 'SUSPENDU' ? '#fb923c'
            : '#94a3b8';
        return {
            id: p.id, numeroPolice: p.numeroPolice || '—',
            produitNom: p.produit?.nom || '—', produitCode: code,
        ...
        for (const c of all) {
            if (c.expiringSoon) {
                a.push({ icon:'fa-solid fa-clock', color:'#e67e22', bg:'rgba(230,126,34,0.12)',
                    message: `Police ${c.numeroPolice} expire bient�t`, routerLink: `/mon-espace/polices/${c.id}` });
            }
        }
        this.alerts = a;
    }
